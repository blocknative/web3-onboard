...
import { RequestOptions, SimPlatformResponse, TransactionPreviewInitOptions } from './types.js'

async function simTransaction(options: Omit<TransactionPreviewInitOptions, 'provider'>, transaction) {
  const {apiSecretKey, apiKey} = options
...
const createTransaction = async () => {
  ...
  const inputAmountHex = ethers.BigNumber.from(amount.toString()).toHexString()
...
await createTransaction()
  ...
  const sim = ajax({
    ...
    headers: {
      ...
      credentials: `${apiKey}:${apiSecretKey}`
    },
    body: JSON.stringify(body)
  }).pipe(
    map(response => console.log('response: ', response.response)),
    catchError(error => {
  ...
  return firstValueFrom(zip(sim))
}

export default simTransaction
