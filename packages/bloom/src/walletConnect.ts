...
function walletConnect(walletName: string, options: WalletConnectOptions): WalletInit {
  if (!options.dappUrl) {
    console.warn(
  ...
  return () => {
        return {
            ...
            getInterface: async ({ chains, EventEmitter, appMetadata }) => {
                ...
                const { requiredChains, optionalChains } = buildWCChains(options.requiredChains ?? [], options.optionalChains ?? [], chains)
                const { requiredMethods, optionalMethods } = buildWCMethods(options.additionalRequiredMethods ?? [], options.additionalOptionalMethods ?? [], REQUIRED_METHODS, methods)
