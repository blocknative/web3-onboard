import { createSignal, createEffect, Ref, createMemo, Accessor, Setter, Signal } from "solid-js";
...
function createLocalStorageSignal<T>(
  ...
  const initialValue: T = JSON.parse(storage.getItem(key) ?? "{}").value ?? defaultValue;
...
const [alreadyConnectedWallets, setAlreadyConnectedWallets] = createLocalStore<string[]>(STORAGE_KEYS.LAST_CONNECTED_WALLET, []);
const [lastConnectionTimestamp, setLastConnectionTimestamp] = createLocalStorageSignal('lastConnectionTimestamp', 0)
const [onboardState, setOnboardState] = createStore<AppState>({} as AppState);

const updateAlreadyConnectedWallets = () => {
  setAlreadyConnectedWallets(onboardState.wallets.map((w: WalletState) => w.label))
}
